	...
	function initGroupFunctions(parent) {
		...
		if (undefined !== window.cws_groups) {
			...
			for (var key in window.cws_groups) {
					if (typeof window.cws_groups[key] === 'string' || window.cws_groups[key] instanceof String){

				if (window.cws_groups.hasOwnProperty(key) && undefined !== window.cws_groups[key]) {
					group = JSON.parse(window.cws_groups[key]);
							var param_parent = parent;
							var param_key = key;
							var k = 0;
							//If is Widget
							if (key.indexOf("widget") != -1){
								...
								if (jQuery(parent).siblings('.widget-id').val() == identification){
								...
								if (key.indexOf("]") != -1){
									if (/\[/.test(key)){
										var parent_name = key.match(/^([\w+]+)\[/)[1];
										param_parent = jQuery(parent).find('.'+parent_name);
								...
								k = addGroups(param_parent, group, param_key);
							}

							if (k) {
								window.cws_groups[key] = k;
							}
					i++;
				}
