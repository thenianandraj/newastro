 ...
 */
if(!class_exists('Wt_Import_Export_For_Woo_Order_Basic_Activator_Order')){
	class Wt_Import_Export_For_Woo_Order_Basic_Activator_Order {
		...
		*/
		public static function update_cross_promo_banner_version() {
			...
			if ( false === $current_latest ||  // User is installing the plugin first time.
				version_compare( $current_latest, WBTE_OIEW_CROSS_PROMO_BANNER_VERSION, '<') // $current_latest is lesser than the installed version in this plugin.
			) {
				update_option('wbfte_promotion_banner_version', WBTE_OIEW_CROSS_PROMO_BANNER_VERSION);
			}
		}
		...
		public static function install_tables()
		{
			...
			// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- This is a valid use of direct database query.
			if(!$wpdb->get_results($wpdb->prepare($search_query, $table_name), ARRAY_N)) 
			{
			// phpcs:enable
				// phpcs:disable WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.NotPrepared -- Creating a necessary table.
