...
if(!class_exists('Wt_Import_Export_For_Woo_Order_Basic_Export')){
class Wt_Import_Export_For_Woo_Order_Basic_Export
{
	public $module_id='';
	...
	{	
		$this->default_export_method= Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::get_advanced_settings('default_export_method');
		$this->default_batch_count=Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::get_advanced_settings('default_export_batch');
		$this->use_bom = (bool)Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::get_advanced_settings('include_bom');
	}
	...
	{   
		...
		if(Wt_Iew_Sh::check_write_access(WT_IEW_PLUGIN_ID_BASIC))
		{
			...
			*/
			...
			$ajax_obj = new Wt_Import_Export_For_Woo_Order_Basic_Export_Ajax($this, $this->to_export, $this->steps, $this->export_method, $this->selected_template, $this->rerun_id);
	...
	*/
	public function process_upload($step, $export_id, $to_export)
	{
		...
		{
			$remote_adapter=Wt_Import_Export_For_Woo_Order_Basic::get_remote_adapters('export', $file_into);
	...
	public function process_action($form_data, $step, $to_process, $file_name='', $export_id=0, $offset=0)
	{
		...
		if($is_last_offset) //finished
		{
			...
			}else
			{
				...
				{
						$history_module_id= Wt_Import_Export_For_Woo_Order_Basic::get_module_id('history');
...
Wt_Import_Export_For_Woo_Order_Basic::$loaded_modules['export']=new Wt_Import_Export_For_Woo_Order_Basic_Export();