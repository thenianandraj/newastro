...
if(!class_exists('Wt_Import_Export_For_Woo_Order_Basic_Import')){
class Wt_Import_Export_For_Woo_Order_Basic_Import
{	
	public $module_id='';
     ...
     */
    private static $handled_post_types = array('order', 'coupon', 'subscription');
	...
	{	
		$this->default_import_method= Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::get_advanced_settings('default_import_method');
		$this->default_batch_count=Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::get_advanced_settings('default_import_batch');
	}
	...
	*/
	protected function _process_rerun($rerun_id)
	{
		if($rerun_id>0)
		{
			...
			$history_module_obj=Wt_Import_Export_For_Woo_Order_Basic::load_modules('history');
	...
	protected function enqueue_assets()
	{
        if(Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::wt_is_screen_allowed()){
			...
			if(!is_null($history_module_obj))
			{
				wp_enqueue_script(Wt_Import_Export_For_Woo_Order_Basic::get_module_id('history'),WT_O_IEW_PLUGIN_URL.'admin/modules/history/assets/js/main.js', array('jquery'), WT_O_IEW_VERSION, false);
			}
	...
	public function download_remote_file($form_data)
	{
		...
		if($file_from == 'local' || $file_from=='url') {
			...
			if(is_array($out) && isset($out['response']) && $out['response']) { /* a form validation hook for remote modules */
				...
				$remote_adapter = Wt_Import_Export_For_Woo_Order_Basic::get_remote_adapters('import', $file_from);
	...
	*/
	public function process_action($form_data, $step, $to_process, $file_name='', $import_id=0, $offset=0)
	{
		...
		$reader=new Wt_Import_Export_For_Woo_Order_Basic_Csvreader($csv_delimiter);
		...
		*/
		if(!empty($import_response) && is_array($import_response) && Wt_Import_Export_For_Woo_Order_Basic_Common_Helper::get_advanced_settings('enable_import_log')==1)
		{
	...
	protected function generate_log_summary($data, $is_last_offset)
	{
		...
		if($is_last_offset)
		{
			...
			if(Wt_Import_Export_For_Woo_Order_Admin_Basic::module_exists('history'))
			{
	...
	public function ajax_main()
	{
		...
		if ( Wt_Iew_Sh::check_write_access(WT_IEW_PLUGIN_ID_BASIC ) ) {
			...
			*/
			...
			$ajax_obj=new Wt_Import_Export_For_Woo_Order_Basic_Import_Ajax($this, $this->to_import, $this->steps, $this->import_method, $this->selected_template, $this->rerun_id);
...
Wt_Import_Export_For_Woo_Order_Basic::$loaded_modules['import']=new Wt_Import_Export_For_Woo_Order_Basic_Import();