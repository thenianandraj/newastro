...
(function ($) {
    ...
    $(document).ready(function () {
        // Show popup
        $('.wbte_oimpexp_help-widget_popupover.wbte_oimpexp_help-widget_click ul li:nth-child(4) a').on('click', function (e) {
            ...
            if (!$(this).find('.wt_iew_free_addon_warn').text().trim()) {
                Object.keys(pluginMap).forEach(function (key) {
                    if (pluginMap[key].includes(datType)) {
                        // Update plugin name and review URL
                        // Update help section links
                        var helpWidgetLinks = $('.wbte_oimpexp_help-widget_popupover.wbte_oimpexp_help-widget_click ul a');
