...
if ( ! class_exists( 'Wbte_Accessibility_Banner' ) ) {
	...
	class Wbte_Accessibility_Banner {
		 ...
		 * @var string
		 */
		private static $plugin_prefix = 'wt_o_iew';
		...
		public static function install_plugin() {
			try {
				...
				$response = array(
					'status'  => false,
					'message' => esc_html__( 'Something went wrong. Please try again.', 'order-import-export-for-woocommerce' ),
				);

				if ( ! current_user_can( 'install_plugins' ) ) {
					$response['message'] = esc_html__( 'You do not have sufficient permissions to install plugins.', 'order-import-export-for-woocommerce' );
				...
				if ( ! isset( $api->download_link ) ) {
					$response['message'] = esc_html__( 'Plugin download link not found.', 'order-import-export-for-woocommerce' );
					wp_send_json( $response );
					return;
				}
		...
		public static function activate_plugin() {
			...
			if ( ! current_user_can( 'activate_plugins' ) ) {
				$result['msg'] = esc_html__( 'You do not have sufficient permissions to activate plugins.', 'order-import-export-for-woocommerce' );
				return $result;
			}
			...
			if ( file_exists( WP_PLUGIN_DIR . '/' . self::$plugin_file ) ) {
				...
				if ( is_wp_error( $activation_result ) ) {
					$result['status'] = false;
					$result['msg']    = esc_html__( 'Activation failed. Please try again.', 'order-import-export-for-woocommerce' );
				} else {
					$result['status'] = true;
				}
			}
		...
		public function show_banner_notice() {
			...
			<div id="wbte_accessibility_banner" class="notice">
				<div class="wbte_accessibility_banner_main">
					<img src="<?php echo esc_url( plugin_dir_url( __FILE__ ) . 'assets/images/accessyes.svg' ); ?>" alt="<?php esc_attr_e( 'Accessibility Widget', 'order-import-export-for-woocommerce' ); ?>" style="width: 48px; height: 48px;">
					<div class="wbte_accessibility_banner_content">
						<p class="wbte_accessibility_banner_content_title">
							...
							printf(
								...
								esc_html__( 'Make your site inclusive with %1$s AccessiYes accessibility widget. %2$s', 'order-import-export-for-woocommerce' ),
						...
						<p style="font-size: 14px;">
							<?php
							esc_html_e( 'A lightweight accessibility widget to improve accessibility and usability for every users. Set it up in minutes for free.', 'order-import-export-for-woocommerce' );
							?>
						</p>
					</div>
				</div>
				<div class="wbte_accessibility_banner_actions">
					<button class="button button-primary" id="wbte_accessibility_banner_install_btn">
						...
						<span class="wbte_accessibility_banner_install_btn_text">
							<?php esc_html_e( 'Install & activate', 'order-import-export-for-woocommerce' ); ?>
						</span>
					</button>
					<a href="#" class="wbte_accessibility_banner_maybe_ltr">
						<?php esc_html_e( 'Remind me later', 'order-import-export-for-woocommerce' ); ?>
					</a>
				</div>
				<span class="wbte_accessibility_banner_dismiss_btn">
					<?php esc_html_e( 'Dismiss', 'order-import-export-for-woocommerce' ); ?>
		...
		public static function enqueue_admin_scripts() {
			...
			$error_msg = esc_js( __( 'Something went wrong. Please try again.', 'order-import-export-for-woocommerce' ) );
			?>
			<style>
			...
			( function( $ ) {
				...
				var wbteAccessyesNotify = {
					success: function( message ) {
						...
						} else if ( typeof wbte_oimpexp_notify_msg !== 'undefined' ) {
							wbte_oimpexp_notify_msg.success( message );
						} else {
					...
					error: function( message ) {
						if ( typeof wbte_sc_notify_msg !== 'undefined' ) {
							...
							wbte_oimpexp_notify_msg.error( message );
						} else {
							window.alert( message );
						}
					}
				...
				$( document ).ready( function() {
					...
					function setButtonLoading( isLoading ) {
						if ( isLoading ) {
							...
							btnTextSpan.text( '<?php echo esc_js( __( 'Installing...', 'order-import-export-for-woocommerce' ) ); ?>' );
						...
						$.ajax( {
							...
							success: function( response ) {
								if ( response.status ) {
									...
									wbteAccessyesNotify.success( '<?php echo esc_js( __( 'Plugin installed successfully.', 'order-import-export-for-woocommerce' ) ); ?>' );
								} else {
									setButtonLoading( false );
