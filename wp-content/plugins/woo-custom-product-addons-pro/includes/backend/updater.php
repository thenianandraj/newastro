...
class Updater {
    ...
    public function check_update( $_transient_data ) {
        ...
        if(isset($current->msg)){
            if(substr( $current->msg, 0, 24 ) === "License key is not valid" || substr( $current->msg, 0, 32 )==='No license key has been provided'){
                $settings = \get_option('wcpa_settings_key');
                if(is_array($settings) && !empty($settings)){
                    $settings['wcpa_acd_count']=isset($settings['wcpa_acd_count'])?($settings['wcpa_acd_count']+1):1;
