...
if ( ! class_exists( 'WCPBC_Admin_Notices', false ) ) :
	...
	class WCPBC_Admin_Notices {
		...
		private static function init_notices() {
			if ( ! self::$notices ) {
				...
				$notices = array(
					...
					'wc_payments_multi_currency_disabled' => array( 'hide' => 'yes' ),
				);

				if ( wcpbc_is_pro() && ( ! is_callable( array( 'WCPBC_License_Settings', 'is_license_connected' ) ) || ! WCPBC_License_Settings::is_license_connected() ) ) {
					// Add the license required notice.
					$notices['no_license'] = array( 'hide' => 'no' );
				}

				if ( is_callable( array( 'WCPBC_Admin_Ads', 'admin_notices' ) ) ) {
					// Ads.
					$notices = array_merge( $notices, WCPBC_Admin_Ads::admin_notices() );
				}
		...
		public static function hide_notice() {
			if ( isset( $_GET['pbc-hide-notice'] ) && isset( $_GET['_wpnonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification

				if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_GET['_wpnonce'] ) ), 'pbc_hide_notice_nonce' ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				$hide_notice = wc_clean( wp_unslash( $_GET['pbc-hide-notice'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
		...
		public static function display_no_license_notice() {
			...
			if ( ! ( isset( $_GET['tab'] ) && 'price-based-country' === $_GET['tab'] && isset( $_GET['section'] ) && 'license' === $_GET['section'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				$notice = sprintf( __( '%1$sWarning!%2$s You didn\'t set your %1$sWooCommerce Price Based Country Pro%2$s license key yet, which means you\'re missing out on updates and support! Enter your %3$slicense key%4$s.', 'woocommerce-product-price-based-on-countries' ), '<strong>', '</strong>', '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=price-based-country&section=license' ) ) . '">', '</a>' );
		...
		/**
		 * Output the WooCommerce Payments multi-currency disabled notice.
		 */
		public static function display_wc_payments_multi_currency_disabled_notice() {
			// translators: HTML Tags.
			$message = sprintf( __( '%1$sThe multi-currency option of WooCommerce Payments has been disabled%2$s to prevent issues with the pricing zones. You do not need to enable this option to receive payments in multiple currencies.', 'woocommerce-product-price-based-on-countries' ), '<strong>', '</strong>' );
			$type    = 'info';
			include_once dirname( __FILE__ ) . '/views/html-notice-default.php';

			self::remove_notice( 'wc_payments_multi_currency_disabled' );
		}
	}
