...
function sc_insert (){
	if( window.tinyMCE ) {
		...
		if (this.sc_name == 'custom_list') {
			...
			var bgColor = jQuery(parent).find('.row.bg_color span .wp-color-picker').val();
			var fontColor = jQuery(parent).find('.row.f_color span .wp-color-picker').val();
			var colorCheck = jQuery(parent).find('.row.i_color span .wp-color-picker').val();
			var padding = jQuery(parent).find('.row.ofs_padding input').val();

			var customize_check = jQuery(parent).find('.row.customize_checkmarks input')[1].checked;
			var marginTop = jQuery(parent).find('.row.margin_top input').val();
			var marginBottom = jQuery(parent).find('.row.margin_bottom input').val();

			var li = editor.selection.getNode();
			var ul = li.parentNode;
			var li_text = li.innerText;
			var li_icon = "<i class='list_list "+icon+"'"+('custom_icon_style' === value ? "style='color:"+color+"'" : "")+"></i>";
			var checkMarkIcon = "checkmarks_style" == value ? "<i class='list_list_checkmark' style='color:"+colorCheck+"'></i>" : "";
			var classCheckmark = '';
			if(!customize_check){		
				classCheckmark = 'standardCheckmark';
			}else{
				classCheckmark = 'customCheckmark';
			}
			jQuery(li).addClass(classCheckmark).css({'backgroundColor':bgColor,'color':fontColor,'marginTop': parseInt(marginTop)+"px",'marginBottom':parseInt(marginBottom)+"px",'padding': "0" + " " + parseInt(padding) + "px"}).html(li_icon + checkMarkIcon + li_text);
			...
			if (icon) {
				...
				if (this.sc_selection.length > 0) {
					jQuery(li).css({'backgroundColor':bgColor,'color':fontColor}).html(li_icon + checkMarkIcon + li_text);
					this.ins_button.cws_tb_modal_close();
					return;
				}				
					for (var i = node_len - 1; i >= 0; i--) {
						var li_element =  ul.children[i];
						jQuery(li_element).children('.list_list').remove();	
					}
						for (var i = node_len - 1; i >= 0; i--) {
							var li_element =  ul.children[i];
							jQuery(li_element).prepend( "<i class='list_list "+icon+"'"+('custom_icon_style' === value ? "style='color:"+color+"'" : "")+"></i>" );
						}
			} else {
				for (var i = node_len - 1; i >= 0; i--) {
					var li_element =  ul.children[i];
